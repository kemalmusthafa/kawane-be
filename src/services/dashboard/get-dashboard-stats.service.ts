  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const topProducts = topProductsData.map((item) => {
        const product = topProductsWithDetails.find(
      ...
      const pendingOrders =
        orderStatusCounts.find((item) => item.status === "PENDING")?._count
          .status || 0;
      const completedOrders =
        orderStatusCounts.find((item) => item.status === "COMPLETED")?._count
          .status || 0;
