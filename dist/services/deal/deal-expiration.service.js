...
exports.cleanupExpiredDealsService = exports.expireDealsService = void 0;
const prisma_1 = __importDefault(require("../../prisma"));
const expireDealsService = async () => {
    const now = new Date();
    try {
        // Find expired deals
        const expiredDeals = await prisma_1.default.deal.findMany({
            where: {
                status: "ACTIVE",
                endDate: { lt: now },
            },
        ...
        for (const deal of expiredDeals) {
            ...
            await prisma_1.default.deal.update({
                where: { id: deal.id },
                data: { status: "EXPIRED" },
            });
            // Delete deal product relationships (NO PRODUCTS DELETED - only relationships)
            console.log(`Removing deal relationships for ${deal.dealProducts.length} products`);
            await prisma_1.default.dealProduct.deleteMany({
            ...
            for (const dealProduct of deal.dealProducts) {
                ...
                if (product.name.includes(deal.title) ||
                    product.sku?.startsWith("DEAL-") ||
                    (product.description && product.description.includes("DEAL SPECIAL"))) {
                    console.log(`Deleting deal-specific product: ${product.name} (${product.id})`);
...
const cleanupExpiredDealsService = async () => {
    ...
    try {
        ...
        for (const deal of oldExpiredDeals) {
            ...
            // Delete deal images
            await prisma_1.default.dealImage.deleteMany({
                where: { dealId: deal.id },
            });
...
exports.cleanupExpiredDealsService = cleanupExpiredDealsService;
