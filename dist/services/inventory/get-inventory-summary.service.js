...
async function getInventorySummaryService() {
    ...
    const inventory = products.map((product) => {
        ...
        const lastRestockLog = product.inventoryLogs.find((log) => log.change > 0);
        ...
        return {
            ...
            lastRestock,
            lastActivity: product.inventoryLogs[0]?.createdAt.toISOString().split("T")[0] ||
                product.updatedAt.toISOString().split("T")[0],
            status,
            category: product.category?.name || "No Category",
            lastActivityNote: product.inventoryLogs[0]?.note || "No recent activity",
        };
    });
    return {
        inventory,
