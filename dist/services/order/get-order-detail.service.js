...
const getOrderDetailService = async (params) => {
    ...
    const transformedItems = order.items.map((item) => {
        ...
        if (!item.size && item.product.sizes && item.product.sizes.length > 0) {
            ...
            const mostLikelySize = sizePriority.find((size) => availableSizeNames.includes(size));
            if (mostLikelySize) {
    ...
    console.log("📦 Order items from database:", order.items.map((item) => ({
        id: item.id,
    ...
    console.log("📦 Transformed order items:", transformedItems.map((item) => ({
        id: item.id,
        productName: item.productName,
        quantity: item.quantity,
        size: item.size,
        availableSizes: item.availableSizes?.map((s) => s.size).join(", ") || "None",
